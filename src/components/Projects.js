...
<Card
    ...
    style={{ width: 'auto' }}
    cover={
      <img src={p.image.default} alt={"project_image"}/>
    }
    key={key}
    actions={[
      <img src={companyLogo} className={"image-link"} alt={p.footerLink[1].name} title={p.footerLink[1].name} onClick={() => openProjectInNewWindow(p.footerLink[1].url)}/>,
      <i className={"fas fa-share-square fas-link"} title={p.footerLink[0].name} onClick={() => openProjectInNewWindow(p.footerLink[0].url)}></i>
    ]}
  >
